...
export class ShowTaskComponent {
  ...
  constructor(private route: ActivatedRoute, private apiService: ApiService, private appServices: AppService, private formBuilder: FormBuilder) { }
  ...
  fetchTaskResultDetails(): void {
    this.apiService.find_task_results({task: this.taskId}).subscribe(res => {
      if(!!res.result[0]) {
  ...
  fetchDocuments(): void {
    this.apiService.find_documents({reference: this.taskId, type: 'task'}).subscribe(res => {
      if(!!res.result[0]) {
